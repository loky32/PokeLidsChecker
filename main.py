...
class Interfaz(gui_uic[0], gui_uic[1]):
    ...
    def clear_routine(self):
        ...
        self.short_clean_routine()

    def short_clean_routine(self):
    ...
    def update_prefecture_progress(self):
        ...
        if (self.prefectureLids == 0):
            self.short_clean_routine()
            self.listWidgetCities.addItem("No Lids here!")
            self.imageLid.setPixmap(QPixmap('images/unown.png'))
    ...
    def load_ids(self):
        ...
        if selected_city is not None:
            ...
            self.listWidgetIds.clear()
            self.id_list = list()
            if (conn.execute("SELECT COUNT(*) FROM Cities WHERE city_name = ?", (city,)).fetchone()[0] > 0):
